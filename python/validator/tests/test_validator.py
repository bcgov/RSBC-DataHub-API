...
class TestValidator:
    ...
    def test_sample_data_event_issuance_passes_validation(self):
        sample_data = self.get_sample_data('python/common/tests/sample_data/etk/event_issuance.json')
    ...
    def test_sample_data_vt_payment_passes_validation(self):
        sample_data = self.get_sample_data('python/common/tests/sample_data/etk/vt_payment.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_query_passes_validation(self):
        sample_data = self.get_sample_data('python/common/tests/sample_data/etk/vt_query.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_dispute_passes_validation(self):
        sample_data = self.get_sample_data('python/common/tests/sample_data/etk/vt_dispute.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_dispute_status_update_passes_validation(self):
        sample_data = self.get_sample_data('python/common/tests/sample_data/etk/vt_dispute_status_update.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_dispute_finding_passes_validation(self):
        sample_data = self.get_sample_data('python/common/tests/sample_data/etk/vt_dispute_finding.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True
