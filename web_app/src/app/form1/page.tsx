...
export default function Page() {
  const apiSubmitErrorMsg =
    "An error occurred while submitting the application. Please try again or contact RoadSafetyBC by calling 1-855-387-7747 and select option 5. You may also attend a driver licensing office to apply for a review.";
  const pdfEmailSubmitError =
    "An error occurred while submitting pdf email. Please try again or contact RoadSafetyBC by calling 1-855-387-7747 and select option 5. You may also attend a driver licensing office to apply for a review.";

  const router = useRouter();
  const [progress, setProgress] = useState(0);

  const step1Ref = useRef<{ clearData: () => void; validate: () => boolean }>(
    null
  );
  const step2Ref = useRef<{ clearData: () => void; validate: () => boolean }>(
    null
  );
  const step3Ref = useRef<{ clearData: () => void; validate: () => boolean }>(
    null
  );
  const step4Ref = useRef<{ clearData: () => void; validate: () => void }>(
    null
  );
  ...
  const combineError = (errorMsg: string, newError: string) => {
    ...
    else return "There are errors in " + newError;
  };
  ...
  const submitData = () => {
    ...
    if (errorMsg === "") submitDataAfterValidation();
  };
  ...
  /* const submitDataAfterValidation = async () => {
    ...
    if (step1Data.controlIsUl)
      pdfList.push(
        { id: "ulControlBlock", pageNumber: 5 },
        { id: "step4", pageNumber: 6 }
      );
    else if (step1Data.controlIsIrp)
      pdfList.push(
        { id: "irpControlBlock", pageNumber: 5 },
        { id: "revOption", pageNumber: 6 },
        { id: "step4", pageNumber: 7 }
      );
    ...
    const pdf = await generatePDF(
      pdfList,
      "Notice of Driving Prohibition Application for Review"
    );
    ...
    if (file || file !== undefined) reader.readAsDataURL(file);
  }; */
  ...
  return (
    <div
      ...
      style={{ display: "grid", marginTop: "2px", marginRight: "2px" }}
    >
      <div id="formContent">
        <div id="page1img1">
          ...
          <CustomAccordion
            ...
            content={
              ...
              >
                ...
                <p>
                  Submit only <strong> 1 online application</strong> for your
                  prohibition review.
                </p>
                <p>
                  You&apos;ll receive at least <strong> 2 emails</strong> when
                  you submit the form:
                </p>
        ...
        <CustomAccordion
          title="Step 1: Enter Prohibition Information"
          id="step1"
          isExpanded={isExpanded}
          content={
            <Step1
              step1DatatoSend={handleStep1Data}
              ref={step1Ref}
              hasError={false}
            ></Step1>
          }
        ...
        <CustomAccordion
          title="Step 2: Enter Applicant Information"
          id="step2"
          isExpanded={isExpanded}
          content={
            <Step2
              step2DatatoSend={handleStep2Data}
              isEnabled={
                step1Data.licenseSeized === "licenseSeized" ||
                step1Data.controlIsUl
              }
              ref={step2Ref}
              hasError={false}
            ></Step2>
        ...
        <CustomAccordion
          title="Step 3: Complete Review Information"
          id="step3"
          isExpanded={isExpanded}
          content={
            <Step3
              controlIsUl={step1Data.controlIsUl}
              controlIsIrp={step1Data.controlIsIrp}
              controlIsAdp={step1Data.controlIsAdp}
              ref={step3Ref}
              step3DatatoSend={handleStep3Data}
              isEnabled={
                step1Data.licenseSeized === "licenseSeized" ||
                step1Data.controlIsUl
              }
              hasError={false}
              irpProhibitionTypeLength={step1Data.irpProhibitionTypeLength}
            />
          }
        />
              ...
              <div style={{ paddingTop: "30px" }}>
                <strong>
                  <span style={{ fontSize: "16px" }}>
                    Submit only 1 online application for your prohibition
                    review.
                  </span>
                </strong>
              </div>
