...
import {
  ...
  axiosMailItClient,
  axiosApiClient,
} from "../_nonRoutingAssets/lib/form.api";
import {
  ActionResponse,
  Step1Data,
  Step2Data,
...
export async function postForm1(
  ...
  let xmlData = getForm1Xml(step1Data, step2Data, step3data, step4data)
    .replace(/\n/g, "")
    .trim();
    ...
    if (response.status === 200) {
      ...
      return {
        data: {
          is_success: false,
          error:
            "Review could not be schedule at this time, please try again later.",
        },
      };
    }
  } catch (error) {
...
function getForm1Xml(
  ...
  const ulGrounds =
    step3data.ulGrounds && step3data.ulGrounds.length > 0
      ? `<ul-grounds>${step3data.ulGrounds.join(" ")}</ul-grounds>`
      : "<ul-grounds/>";
  const adpGroundsAlcohol =
    step3data.adpGroundsAlcohol && step3data.adpGroundsAlcohol.length > 0
      ? `<adp-grounds-alcohol>${step3data.adpGroundsAlcohol.join(
          " "
        )}</adp-grounds-alcohol>`
      : "<adp-grounds-alcohol/>";
  const adpGroundsDrugs =
    step3data.adpGroundsDrugs && step3data.adpGroundsDrugs.length > 0
      ? `<adp-grounds-drugs>${step3data.adpGroundsDrugs.join(
          " "
        )}</adp-grounds-drugs>`
      : "<adp-grounds-drugs/>";
  const adpGroundsAlcoholDrugs =
    step3data.adpGroundsAlcoholDrugs &&
    step3data.adpGroundsAlcoholDrugs.length > 0
      ? `<adp-grounds-alcohol-drugs>${step3data.adpGroundsAlcoholDrugs.join(
          " "
        )}</adp-grounds-alcohol-drugs>`
      : "<adp-grounds-alcohol-drugs/>";
  const adpGroundsDrugExpert =
    step3data.adpGroundsDrugExpert && step3data.adpGroundsDrugExpert.length > 0
      ? `<adp-grounds-drug-expert>${step3data.adpGroundsDrugExpert.join(
          " "
        )}</adp-grounds-drug-expert>`
      : "<adp-grounds-drug-expert/>";
  const adpGroundsRefusal =
    step3data.adpGroundsRefusal && step3data.adpGroundsRefusal.length > 0
      ? `<adp-grounds-refusal>${step3data.adpGroundsRefusal.join(
          " "
        )}</adp-grounds-refusal>`
      : "<adp-grounds-refusal/>";
  const irpGroundsList =
    step3data.irpGroundsList && step3data.irpGroundsList.length > 0
      ? `<irp-grounds-list>${step3data.irpGroundsList.join(
          " "
        )}</irp-grounds-list>`
      : "<irp-grounds-list/>";
  const licenceNotSurrendered = step1Data.licenseNoSurrendered
    ? `<licence-not-surrendered>${step1Data.licenseNoSurrendered}</licence-not-surrendered>`
    : "<licence-not-surrendered/>";
  const licenceLostOrStolen = step1Data.licenseLostOrStolen
    ? `<licence-lost-or-stolen>${step1Data.licenseLostOrStolen}</licence-lost-or-stolen>`
    : "<licence-lost-or-stolen/>";
  const licenceNotIssued = step1Data.licenseNotIssued
    ? `<licence-not-issued>${step1Data.licenseNotIssued}</licence-not-issued>`
    : "<licence-not-issued/>";
  const licenseSeized = step1Data.licenseSeized
    ? `<licence-seized>licence-seized</licence-seized>`
    : `<licence-seized />`;
  const dateOfService = step1Data.dateOfService
    ? `<date-of-service>` +
      step1Data.dateOfService.toISOString().slice(0, 10) +
      `</date-of-service>`
    : (() => {
        throw new Error("Field is required: date-of-service");
      })();
  const driverFirstName = step2Data.driverFirstName
    ? `<driver-first-name>${step2Data.driverFirstName}</driver-first-name>`
    : "<driver-first-name/>";
  const driverLastName = step2Data.driverLastName
    ? `<driver-last-name>${step2Data.driverLastName}</driver-last-name>`
    : "<driver-last-name/>";
  const driverBcdl = step2Data.driverBcdl
    ? `<driver-bcdl>${step2Data.driverBcdl}</driver-bcdl>`
    : "<driver-bcdl/>";
  const hearingRequest = step3data.hearingRequest
    ? `<hearing-request-type>${step3data.hearingRequest}</hearing-request-type>`
    : "<hearing-request-type/>";
  const representedByLawyer = step2Data.representedByLawyer
    ? `<represented-by-lawyer>${step2Data.representedByLawyer}</represented-by-lawyer>`
    : "<represented-by-lawyer/>";
  const applicantRoleSelect = step2Data.applicantRoleSelect
    ? `<applicant-role-select>${step2Data.applicantRoleSelect}</applicant-role-select>`
    : "<applicant-role-select/>";
  const applicantRole =
    step2Data.applicantRoleSelect === "driver" &&
    step2Data.representedByLawyer === "yes"
      ? `<applicant-role>lawyer</applicant-role>`
  ...
  const xmlString = `
        ...
        <form xmlns:fr="http://orbeon.org/oxf/xml/form-runner" fr:data-format-version="4.0.0">
            ...
            <prohibition-information>
                ...
                <prohibition-number-clean>${
                  step1Data.prohibitionNumberClean
                }</prohibition-number-clean>
                ...
                <irp-prohibition-type-length>${
                  step1Data.irpProhibitionTypeLength
                }</irp-prohibition-type-length>
                ${dateOfService}
            </prohibition-information>
    ...
    step2Data.sendConsentSeparately
                ...
                <applicant-email-address>${
                  step2Data.applicantEmailAddress
                }</applicant-email-address>
                <appeals-registry-email>${
                  process.env.APPEALS_REGISTRY_EMAIL
                }</appeals-registry-email>
                ...
                <do-not-reply-address>${
                  process.env.DO_NOT_REPLY_ADDRESS
                }</do-not-reply-address>
                ...
                <control-driver-postal-code>${step2Data.controlDriverPostalCode.replace(
                  " ",
                  ""
            ...
            <consent-and-submission>
                <signature-applicant-name>${
                  step4data.signatureApplicantName
                }</signature-applicant-name>
