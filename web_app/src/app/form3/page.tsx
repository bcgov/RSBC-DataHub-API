...
export default function Page() {
  const submitErrorMsg =
    "An error occurred while submitting the application. Please try again at a later time.";
  ...
  const step4Ref = useRef<{ clearData: () => void; validate: () => boolean }>(
    null
  );
  const [validProhibitionNumber, setValidProhibitionNumber] = useState(false);
  const [prohibitionNumberErrorText, setProhibitionNumberErrorText] =
    useState("");
  ...
  const [signatureApplicantNameErrorText, setSignatureApplicantNameErrorText] =
    useState("");
  ...
  const validateField = () => {
    if (prohibitionNumberRegex.exec(applicantInfo.controlProhibitionNumber)) {
      ...
      setApplicantInfo({
        ...
        controlIsIrp:
          applicantInfo.controlProhibitionNumber.startsWith("21") ||
          applicantInfo.controlProhibitionNumber.startsWith("40"),
  ...
  const driverLastNameChanged = (e: React.ChangeEvent<HTMLInputElement>) => {
    setApplicantInfo({
      ...applicantInfo,
      controlDriverLastName: e.target.value,
    });
  };
  ...
  async function submitData() {
    ...
    if (validateData()) {
      ...
      try {
        ...
        const emailResult = await sendForm3Email(
          filesContent,
          filesNames,
          applicantInfo
        );
        ...
        if (formSubmitted && emailResult === 202) {
          ...
          setMessage(
            "Your documents are sent. Please check your email. If you would like a copy of this form, click the PDF button."
          );
          setIsLoading(false);
        } else {
  ...
  useEffect(() => {
    setTooltipContent({
      tooltipContent1:
        "Enter first 8 numbers with the dash.Don't enter the digit in the grey box. Prohibition numbers start with 00, 21, 30 or 40.",
      tooltipContent2:
        "Enter the last name on your driver's prohibition number form.",
      tooltipContent3:
        "You can submit your evidence, or a lawyer or person you authorize can do it on your behalf.",
      tooltipContent4:
        "Please enter an email address for communication with RoadSafetyBC.",
  ...
  function validateData() {
    ...
    console.log(
      ...
      isApplicantValid +
        "-" +
        prohibitionNumberErrorText +
        "-" +
        driverLastNameErrorText +
        "-" +
  ...
  function formHasError() {
    console.log(
      "formHasError: " +
        ...
        isValidComboErrorText +
        "-" +
        (prohibitionNumberErrorText + "-" + driverLastNameErrorText ||
          applicantRoleErrorText ||
          emailAddressErrorText ||
          confirmEmailErrorText ||
          fileUploadErrorText ||
  ...
  const callValidateFormData = async () => {
    try {
      ...
      setApplicantInfo({
        ...applicantInfo,
        isProhibitionNumberValid: result.data.is_success,
      });

      if (!result.data.is_success)
        setIsValidComboErrorText("Error: " + result.data.error);
      else setIsValidComboErrorText("");
    } catch (error) {
      setIsValidData(false);
  ...
  const handleApplicantRoleSelect = (
  ...
  const validateEmailAddress = (value: string, field: string) => {
    ...
    if (field === "applicantEmailAddress") {
      if (value)
        emailAddressRegex.exec(value)
          ? setEmailAddressErrorText("")
          : setEmailAddressErrorText(
              "Incorrect email format, enter in format name@example.com"
            );
      else setEmailAddressErrorText("Please enter a valid email address.");
    } else if (value)
      value && value === applicantInfo.applicantEmailAddress
        ? setConfirmEmailErrorText("")
        : setConfirmEmailErrorText("Missing or incorrect value");
    else
      setConfirmEmailErrorText(
        "Please enter the email again to confirm it's the same as the one above."
      );
  };
  ...
  const handleStep4Data = (step4Data: Step4Data) => {
    if (!step4Data.signatureApplicantName)
      setSignatureApplicantNameErrorText(
        "Please enter your name to confirm the information submitted is correct."
      );
    else
      setApplicantInfo({
        ...applicantInfo,
        signatureApplicantName: step4Data.signatureApplicantName,
      });
  };
  ...
  const validateFiles = (files: FileList): boolean => {
    ...
    if (files && files.length > 0) {
      for (let i = 0; i < files.length; i++) {
        ...
        reader.onload = async function (event) {
          ...
          if (await checkVirusScanner(fileContent)) {
            ...
            setFileUploadErrorText(
              "There is problem with a document. Please recheck the documents"
            );
            allFilesValid = false;
          }
      ...
      if (allFilesValid) setFileUploadErrorText("");
  ...
  return (
    <div className="formContent">
      <div id="formContent">
            ...
            content={
              ...
              >
                ...
                <ol style={{ paddingLeft: "30px" }}>
                  <li>
                    <strong>
                      <span style={{ fontSize: "16px" }}>
                        Enter the prohibition number and last name from the
                        notice
                      </span>
                    </strong>
                  </li>
                  ...
                  <li>
                    <span style={{ fontSize: "16px" }}>Click Send</span>
                  </li>
                </ol>

                <div>
                  <span style={{ fontSize: "16px" }}>
                    You&apos;ll receive a confirmation email if you attached
                    your file(s).&nbsp;
                  </span>
                </div>
                <div>
                  <span style={{ fontSize: "16px" }}>
                    If you don&apos;t enter anything in the form for 15 minutes,
                    it may time out.
                  </span>
                </div>
              </div>
        ...
        <CustomAccordion
          title="Step 1: Confirm Applicant Information"
          id="step1"
            ...
            <div style={{ display: "grid", marginTop: "20px" }}>
              <div id="page1img2">
                ...
                >
                  <TextField
                    key="key1"
                    id="control-prohibition-number-field"
                    style={{ paddingLeft: "5px" }}
                    variant="outlined"
                    value={applicantInfo.controlProhibitionNumber}
                    onChange={prohibitionNumberChanged}
                    onBlur={validateField}
                  ></TextField>
                </FormField>
                <div style={{ marginTop: "-30px", marginBottom: "30px" }}>
                  <Image
                    src="/assets/images/Combo_prohibition_no.png"
                    width={280}
                    height={180}
                    alt="Info"
                  />
                ...
                >
                  <TextField
                    id="driver-last-name-field"
                    style={{ paddingLeft: "5px" }}
                    variant="outlined"
                    name="driverLastName"
                    value={applicantInfo.controlDriverLastName}
                    onChange={driverLastNameChanged}
                    ...
                    >
                      ...
                      <Grid container spacing={2} sx={{ paddingTop: "10px" }}>
                        <Grid item xs={5} sx={{ padding: "1px" }}>
                          ...
                          >
                            <RadioGroup
                              id="applicant-role-select-field"
                              name="applicantRoleSelect"
                              value={applicantInfo.applicantRoleSelect}
                              onChange={handleApplicantRoleSelect}
                            >
                              <FormControlLabel
                                value="driver"
                                control={
                        ...
                        <Grid item xs={7} sx={{ padding: "1px" }}>
                          {(applicantInfo.applicantRoleSelect === "lawyer" ||
                            applicantInfo.applicantRoleSelect ===
                              "advocate") && (
                              ...
                              <div>
                                <div style={{ display: "inline-grid" }}>
                                  <span style={{ fontSize: "14px" }}>
                                    <strong>
                                      {" "}
                                      If you haven&apos;t submitted a signed
                                      consent from the driver authorizing you to
                                      send and receive documents on their
                                      behalf.{" "}
                                    </strong>
                                  </span>
                                </div>
                    ...
                    <Grid item xs={5} sx={{ padding: "1px" }}>
                      ...
                      >
                        <TextField
                          ...
                          value={applicantInfo.applicantEmailAddress}
                      ...
                      >
                        <TextField
                          id="cnf-email-address-field"
                          style={{ paddingLeft: "5px" }}
                          variant="outlined"
                          name="applicantEmailConfirm"
                          value={applicantInfo.applicantEmailConfirm}
                          onChange={handleChange}
                          onBlur={handleBlurEmailAddress}
                        ></TextField>
                      </FormField>
        ...
        <div id="page2">
          {isValidData && (
            <div>
              <CustomAccordion
                title="Step 2: Read the Guidelines"
                id="step2"
                isExpanded={isExpanded}
                content={
                  <div
                  ...
                  >
                    ...
                    >
                      <li>
                        You must provide the information you want considered{" "}
                        <strong>48 hours</strong> before your scheduled review.
                      </li>
                      <li>
                        The information you&apos;ll provide should address the
                        grounds you chose on your application.
                      </li>
                      <li>
                        The adjudicator won&apos;t consider evidence you submit
                        after the scheduled review date.
                      </li>
              ...
              <CustomAccordion
                title="Step 3: Attach Statement and Evidence"
                id="step3"
                isExpanded={isExpanded}
                content={
                  <div
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        <li>
                          ...
                          longer to upload your files.{" "}
                        </li>
                        <li>
                          You can upload as many files as you need, but the
                          maximum size for a single file is 25MB.
                        </li>
                        <li>
                          The maximum total size for all the files in a single
                          upload is 50MB.
                        </li>
                        <li>
                          You can return to this form to upload more files until
                          48 hours before your review.
                        </li>
                        <li>
                          A statement must not exceed 30 pages total, for more
                    ...
                    <div>
                      ...
                      <div>
                        {" "}
                        We will not accept cloud or Google docs for evidence.{" "}
                      </div>
                    </div>
                    <Grid item xs={7} sx={{ padding: "1px" }}>
                      <FormField
                      ...
                      {fileNamesMessage && (
                        <div id="messageDiv">Attached: {fileNamesMessage}</div>
                      )}
                    </Grid>
                  </div>
                }
              ...
              <div>
                <Grid item xs={5} sx={{ padding: "1px" }}>
                  <CustomAccordion
                    title="Step 4: Consent and Submit"
                    id="step4"
                    isExpanded={isExpanded}
                    content={
      ...
      <div style={{ paddingBottom: "40px" }}>
        <Grid container spacing={2}>
          <Grid item xs={6} sx={{ padding: "1px" }}></Grid>{" "}
          <Grid item xs={6} sx={{ padding: "1px" }}>
            ...
            <Button
              ...
              startIcon={<Print sx={{ fontWeight: "bold" }} />}
            >
              PDF
            </Button>
            <Button
