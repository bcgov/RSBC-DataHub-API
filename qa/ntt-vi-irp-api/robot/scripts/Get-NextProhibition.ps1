...
Set-Variable -Name "MEMORY_FILE"         -Value "${MEMORY_FOLDER}\next-prohibition-number-${ENVIRONMENT}.txt"
Set-Variable -Name "MEMORY_FILE_ADP"     -Value "${MEMORY_FOLDER}\next-prohibition-number-adp-${ENVIRONMENT}.txt"
Set-Variable -Name "MEMORY_FILE_IRP"     -Value "${MEMORY_FOLDER}\next-prohibition-number-irp-${ENVIRONMENT}.txt"
Set-Variable -Name "MEMORY_FILE_UL"      -Value "${MEMORY_FOLDER}\next-prohibition-number-ul-${ENVIRONMENT}.txt"
Set-Variable -Name "MEMORY_FILE_IMP"     -Value "${MEMORY_FOLDER}\next-prohibition-number-imp-${ENVIRONMENT}.txt"
...
if (-Not [string]::IsNullOrEmpty(${NUMBER_TYPE}))
{
    ...
    # e.g. "ADP"
        ...
        elseif (${NUMBER_TYPE}.ToUpper() -eq "IRP") {
            Set-Variable -Name "MEMORY_FILE" -Value ${MEMORY_FILE_IRP}
...
while(1)
{
    ...
    try {
        ...
        Invoke-WebRequest -UseBasicParsing -Method GET -Headers ${Headers} "${URL}" | Out-Null
    } 
    catch {
        # An HTTP 404 indicates an available prohibition number. This is what we want.
