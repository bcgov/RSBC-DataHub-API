# Get a prohibition record from DEV
<#
.SYNOPSIS
    Retrieve prohibition from VIPS using the DF VI-IRP API.

.DESCRIPTION
    Calls the DF VI-IRP API endpoint GET /v1/prohibitions/correlation endpoint to retrieve a VIPS prohibition.
...
.EXAMPLE
PS> .\Get-Prohibition.ps1 22197503

#>

param (
    [string] $ProhibitionNumber,      # Example: 00197501
    [string] $Environment,
    [string] $ServerUri,
    [switch] $Format,
    [switch] $Verbose
...
function Get-Prohibition {
    ...
    Begin {
            ...
            Set-Variable -Name "Correlation" -Value "GetProhibition"
        ...
        if (-Not ${ServerUri}) {
            Set-Variable -Name "ServerUri" -Value "https://digitalforms-viirp-api-${OcNamespace}-${Environment}.apps.silver.devops.gov.bc.ca/digitalforms-viirp/${ApiPath}/${ProhibitionNumber}/${Correlation}"
        } 
...
if (${Verbose}) {
    $StatusCode, $Response = Get-Prohibition -ServerUri ${ServerUri} -Environment ${Environment} -ProhibitionNumber ${ProhibitionNumber} -Verbose
}
else {
