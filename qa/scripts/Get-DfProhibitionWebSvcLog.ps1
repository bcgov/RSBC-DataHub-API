...
function Watch-DfSvcLog {
    ...
    Process {
        if (${Match}) {
            oc --namespace=${Namespace} logs svc/${ServiceName} | Select-String -NotMatch -Pattern InsecureRequestWarning | Select-String -Pattern ${Match}
        }
        else {
...
Watch-DfSvcLog -Namespace "${OcLicencePlate}-${Environment}" -ServiceName "rsbc-dh-prohibition-web-svc-${Environment}" -Match ${Match}
