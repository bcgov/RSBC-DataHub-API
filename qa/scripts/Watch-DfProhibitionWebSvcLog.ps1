...
function Watch-DfSvcLog {
    ...
    Process {
        oc --namespace=${Namespace} logs svc/${ServiceName} --follow | Select-String -NotMatch -Pattern InsecureRequestWarning
    }    
}
...
Watch-DfSvcLog -Namespace "${OcLicencePlate}-${Environment}" -ServiceName "rsbc-dh-prohibition-web-svc-${Environment}"
